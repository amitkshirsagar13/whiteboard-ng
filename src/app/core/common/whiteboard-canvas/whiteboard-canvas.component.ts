...
export class WhiteboardCanvasComponent implements OnInit {
  ...
  private captureEvents(canvasEl: HTMLCanvasElement) {
    ...
    fromEvent(canvasEl, 'mousedown')
      ...
      .subscribe((res: [MouseEvent, MouseEvent]) => {
        ...
        let oldP = { clientX: prevPos.x, clientY: prevPos.y };
        this.prepareData(newP, oldP);
  ...
  private prepareData(newP: any, oldP: any) {
    let cX = newP.clientX - this.canvas.nativeElement.offsetLeft;
    let cY = newP.clientY - this.canvas.nativeElement.offsetTop;
    let newE = { x: cX, y: cY };
    let oldE = { x: cX, y: cY };
    this.socketService.draw({ oPos: oldE, nPos: newE });
  }

  private drawSync(e) {
    console.log(JSON.stringify(e));
    this.cx.moveTo(e.oPos.x, e.oPos.y)
    this.cx.strokeStyle = '#333333';
    this.cx.lineTo(e.nPos.x, e.nPos.y);
    this.cx.stroke();
  }
}
